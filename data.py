...
def split(Graph):
    ...
    test_dataset = split_data(Graph, train_num, test_num)
...
#             adj[index] = np.zeros((len(Nodes[index]), len(Nodes[index])), dtype=np.float32())
#             fea[index] = np.zeros((len(Nodes[index]), 2))
#             for l in dy_links_single[index]:
#                 adj[index][Nodes[index].index(l[0]), Nodes[index].index(l[1])] = l[2]
#             for n in range(len(Labels[index])):
#                 fea[index][n][Labels[index][n]] = 1
#             if len(Nodes[index]) > max_node:
#                 max_node = len(Nodes[index])
#             node_num.append(len(Nodes[index]))
#         else:
...
#     return adj, fea, max_node, node_num
...
#             adj, fea, node_num, batch_node_num = dynamic_G(node, Labels[Nodes.index(node)], max_link, his)
#             # node_num = adj.shape[0]
#             for i in range(len(adj)):
#                 if max_n is None:
#                     if batch_node_num[i] > max_num:
#                         max_num = batch_node_num[i]
#                 if batch_node_num[i] > max_num:
#                     adj[i] = adj[i][0:max_num,0:max_num]
#                     fea[i] = fea[i][0:max_num]
#                     batch_node_num[i] = max_num
#             A[-1].append(adj)
#             L[-1].append(Labels[Nodes.index(node)])
#             F[-1].append(fea)
...
def dynamic_links(node, label,max_num, his = False):
    ...
    # for index in range(len(dy_links)):
    #     if len(dy_links[index]) > 0:
    #         dy_links_single.append(sorted(multi2single(dy_links[index]), key=lambda x: x[2], reverse=True)[:max_num])
    #     else:
    #         dy_links_single.append([])
    # 同一时刻求和
