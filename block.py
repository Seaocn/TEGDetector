...
def evaluate_dynamic(dataset, batch_num, model, args, name='Validation', max_num_examples=None):
    ...
    for batch_idx in range(batch_num):
        ...
        print('#####true_label#####',labels[-1])
        print('*****pred_label****',indices)
    ...
    for epoch in range(args.num_epochs):
        ...
        if test_dataset is not None:
            ...
            if test_result['acc'] > best_test_result['acc'] - 1e-7:
                ...
                # mth = os.path.join('/home/NewDisk/zhangdunjie/xhy/TEGDetect/model_param/TEGD-train_rate_0.6_lr_0.01.pth')
...
def train_phishing_detector(train_dataset,model, train_num, val_num, test_num, args, val_dataset = None,test_dataset=None, writer=None ):
    ...
    for epoch in range(args.num_epochs):
        ...
        if epoch % 20 == 0 or (test_result['acc'] > best_test_result['acc'] - 1e-7):
            model_state = {'net': model.state_dict(), 'optimizer': optimizer.state_dict(), 'epoch': epoch}
            # mth = os.path.join(args.logdir, gen_prefix(args), str(epoch) + '.pth')
            mth = os.path.join('/home/NewDisk/zhangdunjie/xhy/TEGDetect/model_param/TEGD-max.pth')
            torch.save(model_state, mth)
...
def main():
    ...
    all_node_path = "./data/graph_nodes_2000_.txt"
    test_node_path = "./data/graph_test_nodes_2000.txt"
    data_path = "./data_1000(1k).npz"
    test_data_path = "./test_data_1000(1.5).npz"
    ...
    # Graph_train_set = con_Gset(all_node_path, data_path, args.batch_size, args.max_links, max_n = max_num_nodes)
    ...
    #
    _, val_accs = train_phishing_detector1(train_dataset, model, train_num, val_num, test_num, args, val_dataset=None, test_dataset=test_dataset,
                        writer=writer)


    # mth = os.path.join(args.logdir, gen_prefix(args), str(97) + '.pth')
    # checkpoint = torch.load(mth)
    # model.load_state_dict(checkpoint['net'])
    # evaluate(test_dataset, test_num, model, args, name='Test')
