...
for i in range(10):
    ...
    # out_tensor = torch.zeros(batch_size, 1000)
    # for i, mask in enumerate(packed_masks):
    #     out_tensor[i, :batch_num_nodes[i]] = mask
    # aa = out_tensor.numpy()
    # a = out_tensor.unsqueeze(2).cuda()
    # aaa = out_tensor.unsqueeze(2).numpy()
    # writer_batch_idx = [0, 3, 6, 9]
    ...
    for epoch in range(args.num_epochs):
        ...
        for batch_idx, data in enumerate(train_dataset):
            ...
            #         train_dataset) // 2 and args.method == 'soft-assign' and writer is not None:
            #     log_assignment(model.assign_tensor, writer, epoch, writer_batch_idx)
            #     if args.log_graph:
            #         log_graph(adj, batch_num_nodes, writer, epoch, writer_batch_idx, model.assign_tensor)
