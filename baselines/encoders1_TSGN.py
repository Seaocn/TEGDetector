...
def csr_to_2000array(matrix):
    ...
    if Array.shape[0]<2000:
        x=np.zeros((Array.shape[0],int(2000-Array.shape[0])))
...
class SoftPoolingGcnEncoder(GcnEncoderGraph):
    def __init__(self, max_num_nodes, input_dim, hidden_dim, embedding_dim, label_dim, num_layers,
        ...
        # self.gru_1 = GRUCell(max_num_nodes, self.pred_input_dim * (num_pooling + 1), True)
    ...
    def forward(self, x, adj, batch_num_nodes, **kwargs):
        if 'assign_x' in kwargs:
            ...
            x_a = x
        # A = adj
        # a=csr_to_2000array(adj[0])

        A = torch.cat([torch.unsqueeze(torch.from_numpy(csr_to_2000array(adj[j])).float(), 0) for j in range(len(adj))], 0).float().cuda()
        X = torch.cat([torch.unsqueeze(torch.from_numpy(csr_to_2000fea(x[j])).float(), 0) for j in range(len(x))],
                      0).float().cuda()

        # mask
        max_num_nodes = A.size()[1]
        # if batch_num_nodes is not None:
        ...
        embedding_tensor = self.gcn_forward(X, A,
        ...
        for i in range(self.num_pooling):
            ...
            adj = torch.transpose(self.assign_tensor, 1, 2) @ A @ self.assign_tensor
            x_a = x

            embedding_tensor = self.gcn_forward(x, adj,
                                                self.conv_first_after_pool[i], self.conv_block_after_pool[i],
                                                self.conv_last_after_pool[i], concat=True)
...
class diffpool(GcnEncoderGraph):
    ...
    def forward(self, x, adj, batch_num_nodes, Is_attack=False, **kwargs):
        ...
        for n_p in range(self.num_pooling):
            ...
            #A不好pool，选用原有的方法Pool,X选择平均或者最大
            if self.pool_method == 'ave':
                x = []
                for i in range(embedding_mask.shape[0]):
                    x_intel =  self.embedding_GRU[i]
                    #x_intel[2]   维度2000转为200
                    x_intel_list = []
                    for m in range(200):
                        x_intel_list.append(torch.mean(x_intel[:,0 + m*10:10 + m*10,:],dim=1))
                    x_int = torch.cat([torch.unsqueeze(x_intel_list[p], 1) for p in range(len(x_intel_list))], 1)
                    x.append(x_int)
                X_pool = torch.cat([torch.unsqueeze(x[i], 0) for i in range(len(x))], 0)
                ...
                for i in range(embedding_mask.shape[0]):
                    ...
                    x_intel_list = []
                    for m in range(200):
                        x_intel_list.append(torch.max(x_intel[:, 0 + m * 10:10 + m * 10, :], dim=1)[0])
                    x_int = torch.cat([torch.unsqueeze(x_intel_list[p], 1) for p in range(len(x_intel_list))], 1)
                    x.append(x_int)
                X_pool = torch.cat([torch.unsqueeze(x[i], 0) for i in range(len(x))], 0)
...
class TEGD_ave(GcnEncoderGraph):
    def __init__(self, max_num_nodes, input_dim, hidden_dim, embedding_dim, label_dim, num_layers,
                 assign_hidden_dim, assign_ratio=0.25, assign_num_layers=-1, num_pooling=1, pool_method = 'diffpool',
                 pred_hidden_dims=[50],embeding_out=200, concat=True, bn=True, dropout=0.0, linkpred=True,
                 assign_input_dim=-1, args=None):
        '''
        Args:
            num_layers: number of gc layers before each pooling
        ...
        super(TEGD_ave, self).__init__(input_dim, hidden_dim, embedding_dim, label_dim,
                                       num_layers,pred_hidden_dims=pred_hidden_dims, concat=concat,
                                       args=args)
        add_self = not concat
        self.num_pooling = num_pooling
        ...
        self.pred_model = self.build_pred_layers(embeding_out, pred_hidden_dims,
                                                 label_dim, num_aggs=self.num_aggs)
    ...
    def forward(self, x, adj, batch_num_nodes, Is_attack=False, **kwargs):
        ...
        for n_p in range(self.num_pooling):
            ...
            if embedding_mask is not None:
                for i in range(embedding_mask.shape[0]):
                    self.assign_tensor_all.append(torch.mean(self.assign_tensor * embedding_mask[i],dim=1))

                h = torch.cat([torch.unsqueeze(self.assign_tensor_all[i], 0) for i in range(len(self.assign_tensor_all))], 0)
            ...
            for i in range(h.shape[0]):
                ...
                for j in range(h.shape[0]):
                    # print(self.embedding_GRU[i][j])
                    t_e.append(h[i][j] * self.atten_time[j])
                time_embedding.append(torch.cat([torch.unsqueeze(t_e[i], 0) for i in range(len(t_e))], 0))# torch.cat([torch.unsqueeze(self.embedding_GRU_pool_1[0][i],0) for i in range(len(self.embedding_GRU_pool_1))], 0)
                # print(1)
            self.embedding_GRU_pool_1 = torch.cat([torch.unsqueeze(time_embedding[i], 0) for i in range(len(time_embedding))], 0)
            ###
    ...
    def loss(self, pred, label, adj=None, batch_num_nodes=None, adj_hop=1):
        ...
        '''
        eps = 1e-7
        loss = super(TEGD_ave, self).loss(pred, label)
        if self.linkpred:
            max_num_nodes = adj.size()[1]
            pred_adj0 = self.assign_tensor @ torch.transpose(self.assign_tensor, 1, 2)
            tmp = pred_adj0
...
class TEGD_max(GcnEncoderGraph):
    def __init__(self, max_num_nodes, input_dim, hidden_dim, embedding_dim, label_dim, num_layers,
                 assign_hidden_dim, assign_ratio=0.25, assign_num_layers=-1, num_pooling=1, pool_method = 'diffpool',
                 pred_hidden_dims=[50],embeding_out=200, concat=True, bn=True, dropout=0.0, linkpred=True,
                 assign_input_dim=-1, args=None):
        ...
        super(TEGD_max, self).__init__(input_dim, hidden_dim, embedding_dim, label_dim,
                                       num_layers,pred_hidden_dims=pred_hidden_dims, concat=concat,
                                       args=args)
        add_self = not concat
        self.num_pooling = num_pooling
    ...
    def forward(self, x, adj, batch_num_nodes, Is_attack=False, **kwargs):
        ...
        for n_p in range(self.num_pooling):
            ...
            if embedding_mask is not None:
                for i in range(embedding_mask.shape[0]):
                    self.assign_tensor_all.append(torch.max(self.assign_tensor * embedding_mask[i],dim=1)[0])
    ...
    def loss(self, pred, label, adj=None, batch_num_nodes=None, adj_hop=1):
        ...
        '''
        eps = 1e-7
        loss = super(TEGD_max, self).loss(pred, label)
        if self.linkpred:
            max_num_nodes = adj.size()[1]
            pred_adj0 = self.assign_tensor @ torch.transpose(self.assign_tensor, 1, 2)
            tmp = pred_adj0
